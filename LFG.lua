...
function LFG.LFGBrowse_Update()
    ...
    -- Evict stale browseFrames entries whose _G widget no longer exists
    -- (can happen after /reload if the frame table outlives the widget).
    for code, frame in next, LFG.browseFrames do
        if not _G["BrowseFrame_" .. frame.code] then
            LFG.browseFrames[code] = nil
        end
    end
                    ...
                    if _iconLeader then
                        ...
                        if lfmEntry then
                            ...
                            local needStr = #needed > 0
                                and (COLOR_WHITE .. ' needs: ' .. table.concat(needed, COLOR_WHITE .. ', '))
                                or  (COLOR_GREEN .. ' group full')
                            LFG.addOnEnterTooltip(_iconLeader,
                                COLOR_HUNTER .. lfmEntry.leader .. needStr, nil, nil, 15, 0)
                        end
                    end
                end
